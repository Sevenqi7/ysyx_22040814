...
class IDU extends Module{
    ...
    regConnectWithStall(io.ID_to_EX_bus.bits.PC             , IF_pc                     , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.Inst           , IF_Inst                   , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.ALU_Data1      , ALU_Data1                 , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.ALU_Data2      , ALU_Data2                 , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.regWriteID     , rd                        , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.regWriteEn     , regWriteEn                , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.memReadEn      , memReadEn                 , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.memWriteEn     , memWriteEn                , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.optype         , opType                    , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.futype         , futype                    , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.rs1_data       , rs1_data                  , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.rs1_id         , rs1                       , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.rs2_data       , rs2_data                  , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.rs2_id         , rs2                       , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.csrWriteEn     , csrWriteEn                , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.bits.csrWriteAddr   , csrWriteAddr              , !io.IF_to_ID_bus.valid)
    regConnectWithStall(io.ID_to_EX_bus.valid               , ID_valid                  , !io.IF_to_ID_bus.valid)
