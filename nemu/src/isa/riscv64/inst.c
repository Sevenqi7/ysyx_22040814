...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->snpc, s->dnpc = s->pc + imm * 2;ftrace_check_jal(s->dnpc, s->snpc, BITS(s->isa.inst.val, 19, 15), rd));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->snpc, s->dnpc = src1 + imm;ftrace_check_jal(s->dnpc, s->snpc, BITS(s->isa.inst.val, 19, 15), rd));
