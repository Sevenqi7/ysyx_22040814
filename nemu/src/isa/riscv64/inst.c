...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(rd) = SEXT(BITS(src1 >> shamt, 31, 0), 32));
  INSTPAT("010000? ????? ????? 101 ????? 00110 11", sraiw  , I, R(rd) = SEXT((int32_t)BITS(src1, 31, 0) >> shamt, 64));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", bltu   , B, s->dnpc = (src1 < src2) ? s->pc + imm : s->snpc);
